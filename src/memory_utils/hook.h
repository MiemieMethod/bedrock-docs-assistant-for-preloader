...
LPAPI int hook(FuncPtr target, FuncPtr detour, FuncPtr *originalFunc,
               HookPriority priority);

LPAPI bool unhook(FuncPtr target, FuncPtr detour);
 ...
 * @return FuncPtr
 */
LPAPI FuncPtr resolveIdentifier(char const *identifier);
...
#define LP_AUTO_REG_HOOK_IMPL(FUNC_PTR, STATIC, CALL, DEF_TYPE, ...)           \
  VA_EXPAND(HOOK_IMPL(                                                         \
      inline memory::HookAutoRegister<DEF_TYPE> DEF_TYPE##AutoRegister,        \
      FUNC_PTR, STATIC, CALL, DEF_TYPE, __VA_ARGS__))

#define LP_MANUAL_REG_HOOK_IMPL(...) VA_EXPAND(HOOK_IMPL(, __VA_ARGS__))

#define LP_STATIC_HOOK_IMPL(...)                                               \
  VA_EXPAND(LP_MANUAL_REG_HOOK_IMPL((*), static, originFunc, __VA_ARGS__))

#define LP_AUTO_STATIC_HOOK_IMPL(...)                                          \
  VA_EXPAND(LP_AUTO_REG_HOOK_IMPL((*), static, originFunc, __VA_ARGS__))

#define LP_INSTANCE_HOOK_IMPL(DEF_TYPE, ...)                                   \
  VA_EXPAND(LP_MANUAL_REG_HOOK_IMPL((DEF_TYPE::*), , (this->*originFunc),      \
                                    DEF_TYPE, __VA_ARGS__))

#define LP_AUTO_INSTANCE_HOOK_IMPL(DEF_TYPE, ...)                              \
  VA_EXPAND(LP_AUTO_REG_HOOK_IMPL((DEF_TYPE::*), , (this->*originFunc),        \
                                  DEF_TYPE, __VA_ARGS__))
 ...
 * DefType::unhook().
 */
#define LP_TYPED_STATIC_HOOK(DefType, type, priority, identifier, Ret, ...)    \
  VA_EXPAND(LP_STATIC_HOOK_IMPL(DefType,                                       \
 ...
 * DefType::unhook().
 */
#define LP_STATIC_HOOK(DefType, priority, identifier, Ret, ...)                \
  VA_EXPAND(                                                                   \
      LP_STATIC_HOOK_IMPL(DefType, , priority, identifier, Ret, __VA_ARGS__))
 ...
 */
#define LP_AUTO_TYPED_STATIC_HOOK(DefType, type, priority, identifier, Ret,    \
                                  ...)                                         \
  VA_EXPAND(LP_AUTO_STATIC_HOOK_IMPL(DefType,                                  \
                                     : public type, priority, identifier, Ret, \
                                       __VA_ARGS__))
 ...
 */
#define LP_AUTO_STATIC_HOOK(DefType, priority, identifier, Ret, ...)           \
  VA_EXPAND(LP_AUTO_STATIC_HOOK_IMPL(DefType, , priority, identifier, Ret,     \
                                     __VA_ARGS__))
 ...
 * DEF_TYPE::unhook().
 */
#define LP_TYPED_HOOK(DEF_TYPE, PRIORITY, TYPE, IDENTIFIER, RET_TYPE, ...)     \
  VA_EXPAND(LP_INSTANCE_HOOK_IMPL(DEF_TYPE,                                    \
 ...
 * DEF_TYPE::unhook().
 */
#define LP_INSTANCE_HOOK(DEF_TYPE, PRIORITY, IDENTIFIER, RET_TYPE, ...)        \
  VA_EXPAND(LP_INSTANCE_HOOK_IMPL(DEF_TYPE, , PRIORITY, IDENTIFIER, RET_TYPE,  \
                                  __VA_ARGS__))
 ...
 */
#define LP_AUTO_TYPED_INSTANCE_HOOK(DEF_TYPE, PRIORITY, TYPE, IDENTIFIER,      \
                                    RET_TYPE, ...)                             \
  VA_EXPAND(LP_AUTO_INSTANCE_HOOK_IMPL(DEF_TYPE,                               \
                                       : public TYPE, PRIORITY, IDENTIFIER,    \
                                         RET_TYPE, __VA_ARGS__))
 ...
 * @see HOOK for usage.
 */
#define LP_AUTO_INSTANCE_HOOK(DEF_TYPE, PRIORITY, IDENTIFIER, RET_TYPE, ...)   \
  VA_EXPAND(LP_AUTO_INSTANCE_HOOK_IMPL(DEF_TYPE, , PRIORITY, IDENTIFIER,       \
                                       RET_TYPE, __VA_ARGS__))