...
LPAPI FuncPtr resolveSymbol(char const* symbol);
LPAPI FuncPtr resolveSignature(char const *signature);
...
constexpr auto virtualCall(void const *self, uintptr_t off, Args &&...args)
    -> RTN {
...
#define CC_CALL(iname, address, Ret, ...) ((Ret(*)(__VA_ARGS__))(address))